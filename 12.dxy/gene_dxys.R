######################################################################################
######################################################################################
##                     This script was written by Linda Hagberg                     ##
##                                 and Alex Hausmann,                               ##
##                                 who we dearly miss                               ## 
######################################################################################
######################################################################################
...
for (hz in hzs){
    ...
    dxys  = read.table(paste0("01.output/site_dxy_",hz,".txt"), header=T, comment.char="")
    genes = read.table("../00.input/bait.positions") #use gene position
    genes <- genes[0:9270,] #only chr. 1
    genes$V1 <- genes$V2
    genes$V2 = NA
    genes$V3 = NA
...
#    View(data.frame(start_pos,end_pos))
    ...
    write.table(genes, file = paste0('01.output/per_gene_dxys_',hz,'.csv'),row.names=FALSE, quote = FALSE, sep = "\t")
