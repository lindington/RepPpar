...
#SBATCH -J dadi_snp_all
#SBATCH --out=dadi_all.out

module load angsd/0.933-gcc8

# count lines in bamfile = number of individuals, extract only number)
ind=$(wc -l /dss/dsslegfs01/pr53da/pr53da-dss-0029/capture/00.input/bamlist_dadi_all.txt | sed -e 's/\s.*$//')
echo "${ind}"
